class Stock < ApplicationRecord
  ...
  def self.new_lookup(ticker_symbol)
    ...
    if time_series
      ...
      stock = new(ticker: ticker_symbol, last_price: closing_price) # since we are  using self. which is a class method we just give new insied of Stock.new.creates a new instance of a stock class , self.new: When you call new inside a class method without explicitly specifying the class name, Ruby implicitly understands it as self.new. Since self refers to the Stock class in this context, new is equivalent to Stock.new.Inside this class method, when you write new(...), it implicitly calls Stock.new(...) because self inside the class method refers to the Stock class.
